   ...
   //     			processing_edges += blkPtr->edgeNum;
		...
		void processOnFPGA()
		{
			...
			for(int j = tupleSize - 1; j >= 0; j--){
				...
				//std::cout << "vetexnum edgenum " << blkVertexNum[0]<< " ," << blkEdgeNum[0] <<std::endl;
...
int main(int argc, char **argv) {
	...
	//CSR - reorder
	{	
		std::vector<int> data(vertexNum, 0);	
     ...
     //    	std::cout << data[index[i]] << std::endl;
    	...
    	int final_num = 0;
    	for(int kk =0; kk < blkNum; kk ++){
    		
    		int num_tmp = 0;
    		for(int i = 0; i < BLK_SIZE; i ++){
    			int v = index[i];
    			for (int j = csr->rpao[v]; j < csr->rpao[v + 1]; j++){
    				if((csr->ciao[j] > kk * BLK_SIZE) && (csr->ciao[j] < (kk + 1) * BLK_SIZE))
    					num_tmp ++;
    			}
    		}
    		if (final_num < num_tmp) final_num = num_tmp;
    	}
    	printf("[INFO] the maximal edges of partition is %d\n", final_num);
	}
