...
__kernel void __attribute__((task)) processEdge(
		...
		__global float4 * restrict tmpVertexProp,
		...
		)
{
	...
	edge_info_t edge_info_buffer[32];
	int cnt = 0;
	while(true){	
		edge_info_t edge_info = read_channel_nb_altera(edgeInfoCh, &valid_data);
		if(valid_data){

			edge_info_buffer[cnt ++] = edge_info;
			
			if(cnt == 32){
				cnt = 0;
			#pragma ivdep
				for(int i = 0; i < 32; i ++){
					edge_info_t edge_info = edge_info_buffer[i];
					...
					int outDeg = edge_info.outDeg;
						...
						//printf("dst %d ",dstVidx.s0 );
					if(outDeg){
						if(dstVidx.s0 != -1)
						#pragma ivdep
							tmpVPropBuffer0[dstVidx.s0 >> LOG2_BRAM_BANK] += ( vPropBuffer[srcVidx] / outDeg);
							//compute(srcVidx, dstVidx.s0, eProp, outDeg, tmpVPropBuffer0, vPropBuffer, count);
						if(dstVidx.s1 != -1)
						#pragma ivdep
							tmpVPropBuffer1[dstVidx.s1 >> LOG2_BRAM_BANK] += ( vPropBuffer[srcVidx] / outDeg);
							//compute(srcVidx, dstVidx.s1, eProp, outDeg, tmpVPropBuffer1, vPropBuffer, count);
						if(dstVidx.s2 != -1)
						#pragma ivdep
							tmpVPropBuffer2[dstVidx.s2 >> LOG2_BRAM_BANK] += ( vPropBuffer[srcVidx] / outDeg);
							//compute(srcVidx, dstVidx.s2, eProp, outDeg, tmpVPropBuffer2, vPropBuffer, count);
						if(dstVidx.s3 != -1)
						#pragma ivdep
							tmpVPropBuffer3[dstVidx.s3 >> LOG2_BRAM_BANK] += ( vPropBuffer[srcVidx] / outDeg);
		...
		if(end_flag == 0xffff && !valid_data && !valid_flag)
		{
				for(int i = 0; i < cnt; i ++){
					edge_info_t edge_info = edge_info_buffer[i];
					int srcVidx = edge_info.vertexIdx;
