...
cl_int varMap(const int &nodeNum, const int &edgeNum){
	...
	statusVec.push_back(localStatus);
	localStatus = clEnqueueSVMMap(queueReadNgbInfo, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 
		(void *)outEdges, sizeof(int) * 2 * edgeNum , 0, NULL, NULL); 
	statusVec.push_back(localStatus);
...
for(int i = 0; i < nodeNum; i++){
	...
	for(int i = 0; i < nodeNum; i++){
		...
		std::vector<int> bankVec0;
		std::vector<int> bankVec1;
		std::vector<int> bankVec2;
		std::vector<int> bankVec3;

		for(int j = 0; j < num; j++){
			int ngbIdx = csr->ciao[start + j];
			if(ngbIdx % 4 == 0)
				bankVec0.push_back(ngbIdx);
			else if (ngbIdx % 4 == 1)
				bankVec1.push_back(ngbIdx);
			else if (ngbIdx % 4 == 2)
				bankVec2.push_back(ngbIdx);
			else if (ngbIdx % 4 == 3)
				bankVec3.push_back(ngbIdx);
		}

		//std::cout << bankVec0.size() << bankVec1.size() << bankVec2.size() << bankVec3.size() << std::endl;
		outEdgesOld = edgesIdx;
		outRowPointerArray[i] = edgesIdx;
		while(!bankVec0.empty() || !bankVec1.empty() || !bankVec2.empty() || !bankVec3.empty()){
			if(!bankVec0.empty())
				{outEdges[edgesIdx ++] = bankVec0.back(); bankVec0.pop_back();}
			else 
				outEdges[edgesIdx ++] = pad;
			if(!bankVec1.empty())
				{outEdges[edgesIdx ++] = bankVec1.back(); bankVec1.pop_back();}
			else 
				outEdges[edgesIdx ++] = pad;
			if(!bankVec2.empty())
				{outEdges[edgesIdx ++] = bankVec2.back(); bankVec2.pop_back();}
			else 
				outEdges[edgesIdx ++] = pad;

			if(!bankVec3.empty())
				{outEdges[edgesIdx ++] = bankVec3.back(); bankVec3.pop_back();}
			else 
				outEdges[edgesIdx ++] = pad;
		}
...
int main(int argc, char ** argv){
	...
	std::string graphName = "rmat-17-32";
	...
	outEdges           = (int*) clSVMAllocAltera(context, 0, sizeof(int) * edgeNum * 2, 1024);
	...
	printf("original edgeNum %d padding num %d total edges after padding %d \n",edgeNum, padding_num,(edgeNum + padding_num));
...
//for(int i = 0; i < 1; i ++){
	...
	//std::cout << activeVertexNum[0] <<" activeVertices," << " FPGA traverse runtime: " << k0Time << std::endl;
