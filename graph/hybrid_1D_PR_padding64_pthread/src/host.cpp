...
class ThreadPool{
	public:
		...
		pthread_t cpuThread[THREAD_NUM];
   ...
   //  			}
		...
		// static void *staticCpuThreadFunc(void *p) {
		// 	static_cast<Threadpool*>(p)->cpuThreadFunc();
		// 	return NULL;
		// }
		void paddingBlk(){
...
{	
	...
	tp->paddingBlk();
	if(platSeclct == 2) 
		tp->init_fpga( blkVec, vertexNum, edgeNum, blkNum);	
