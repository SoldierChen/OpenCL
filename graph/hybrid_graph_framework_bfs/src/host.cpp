	...
	public:
		...
		void processOnFPGA()
		{
			...
			for(int j = 0; j < tupleSize; j++){
				...
				// status = clEnqueueSVMMap(queueReadNgbInfo, CL_TRUE, CL_MAP_READ, 
				// (void *)blkPtr->rpa.data(), sizeof(int) * blkPtr->rpa.size(), 0, NULL, NULL); 
				// checkStatus("enqueue blkRpa.");
				// status = clEnqueueSVMMap(queueReadNgbInfo, CL_TRUE, CL_MAP_READ, 
				// (void *)blkPtr->cia.data(), sizeof(int) * blkPtr->cia.size(), 0, NULL, NULL); 
				// checkStatus("enqueue blkCia");
				// clSetKernelArgSVMPointerAltera(readNgbInfo, 0, (void*)blkPtr->rpa.data());
				// clSetKernelArgSVMPointerAltera(readNgbInfo, 1, (void*)blkPtr->cia.data());
...
int main(int argc, char **argv) {
	...
	std::cout << "[INFO] Software bfs takes " << elapsedTime << " seconds." << std::endl;
	...
	elapsedTime = (end - begin);
	std::cout << "[INFO] Software bfs with partition takes " << elapsedTime << " seconds." << std::endl;
	...
	std::cout << "[INFO] CPU BFS with partition and multi-thread takes " << elapsedTime << " seconds." << std::endl;
	std::cout << "Verify CPU BFS with partition: " << std::endl;									
