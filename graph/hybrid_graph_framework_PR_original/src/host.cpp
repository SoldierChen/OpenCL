...
static void freeResources(){
	...
	if(blkRpaNum)          clSVMFreeAltera(context, blkRpaNum);
		...
		)
{
	...
	checkStatus("enqueue blkRpa.");
		status = clEnqueueSVMMap(queueReadNgbInfo, CL_TRUE, CL_MAP_READ, 
			(void *)blkRpaNum, sizeof(int) * (vertexNum), 0, NULL, NULL); 
	checkStatus("enqueue blkRpaNum.");
	status = clEnqueueSVMMap(queueReadNgbInfo, CL_TRUE, CL_MAP_READ, 
...
void kernelVarUnmap(){
	...
	status = clEnqueueSVMUnmap(queueReadNgbInfo, 
			(void *)blkRpaNum, 0, NULL, NULL); 
	checkStatus("Unmap blkRpaNum.");
	status = clEnqueueSVMUnmap(queueReadNgbInfo, 
		...
		)
{
	...
	clSetKernelArgSVMPointerAltera(readActiveVertices, 1, (void*)blkActiveVertexNum);
		...
		argvi = 0;
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)blkRpa);
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)blkRpaNum);
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)blkCia);
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)blkEdgeProp);
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)outDeg);	
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)blkActiveVertexNum);
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)blkVertexNum);
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)blkEdgeNum);
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)srcRange);
	clSetKernelArgSVMPointerAltera(readNgbInfo, argvi ++, (void*)itNum);
		argvi = 0;
	...
	public:
		...
		void processOnCPU()
		{
			...
			for(int row = 0; row < tupleSize; row++){
				...
				int srcEnd = blkPtr->srcEnd;
       			processing_edges += blkPtr->edgeNum;
      	...
      	//printf("THE TOTAL EDGES OF THIS TASK IS %d \n", total_edges);
		};
		...
		void processOnFPGA()
		{
			...
			for(int j = 0; j < tupleSize; j++){
        		...
        		//total_edges += blkPtr->edgeNum;
				//std::cout << "total edgeNum: " << total_edges << std::endl;
				...
				for(int i = 0; i < blkVertexNum[0]; i++){
					blkRpaNum[i] = blkRpa[i + 1] - blkRpa[i];
				}
		...
		)
{
	...
	if(!vertexProp || !tmpVertexProp || !rpa || !blkRpa || !blkRpaNum
