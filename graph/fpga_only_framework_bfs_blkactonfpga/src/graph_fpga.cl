...
channel int activeVertexEof    __attribute__((depth(4)));
...
channel int multiActiveVertexCh[16]    __attribute__((depth(256)));
channel int multiActiveVertexChEof    __attribute__((depth(4)));
...
{	
	...
	int srcEnd = srcStart + VERTEX_MAX;
  ...
  #pragma unroll 16
  	for(int i = 0; i < activeVertexNum[0]; i++){
		...
		if((vertexIdx >= srcStart) && (vertexIdx < srcEnd))
			write_channel_altera(multiActiveVertexCh[i & 0xf], vertexIdx);
	}
	write_channel_altera(multiActiveVertexChEof, EOF_FLAG);
	//printf("row %d blkActiveVertexNum[%d] %d baseAddr %d , \n",iterNum[0], iterNum[0], blkActiveVertexNum[iteration], baseAddr);
}

__kernel void __attribute__((task)) gatherActiveVertices(
		__global const int* restrict activeVertices, 
		__global const int* restrict activeVertexNum,
		__global int*  restrict srcRange,
		__global const int* restrict iterNum
...
{	
	...
	while(true){
		...
		#pragma unroll 16
			for(int i = 0; i < 16; i ++){
				data_tmp[i] = read_channel_nb_altera(multiActiveVertexCh[i], &data_vald[i]);
			}
	
		for(int i = 0; i < 16; i ++)
			if(data_vald[i]) write_channel_altera(activeVertexCh, data_tmp[i]);
	
		int tmpEndFlag = read_channel_nb_altera(multiActiveVertexChEof, &end_vald);
	...
	write_channel_altera(activeVertexEof, EOF_FLAG);
	//printf("row %d blkActiveVertexNum[%d] %d baseAddr %d , \n",iterNum[0], iterNum[0], blkActiveVertexNum[iteration], baseAddr);
}
...
{	
	...
	while(true){
		...
		int vertexIdx = read_channel_nb_altera(activeVertexCh, &data_vald);
		if(data_vald){
		...
		int tmpEndFlag = read_channel_nb_altera(activeVertexEof, &validFlag);
		if(validFlag) endFlag = tmpEndFlag;	
		if(endFlag == EOF_FLAG && !data_vald) break;
	}
...
{	
	...
	while(true){
		...
		for(int i = 0; i < (dstNum >> 5); i ++){
			...
			tmpVertexProp[(dstStart >> 4) + (i << 1)] = prop_uint16;
			int16 prop1_uint16;
			...
			tmpVertexProp[(dstStart >> 4) + (i << 1) + 1] = prop1_uint16;
    }
}
