...
static cl_command_queue queueGatherActiveVertices;
...
static cl_kernel gatherActiveVertices; 
...
void setKernelEnv(){
	...
	checkStatus("Failed clCreateCommandQueue of queueProcessEdge.");
	queueGatherActiveVertices = clCreateCommandQueue(context, device, CL_QUEUE_PROFILING_ENABLE, &status);
	checkStatus("Failed clCreateCommandQueue of queueProcessEdge.");
	...
	)
{
	...
	gatherActiveVertices = clCreateKernel(program, "gatherActiveVertices", &status);
	checkStatus("Failed clCreateKernel processEdge.");
	...
	clSetKernelArgSVMPointerAltera(readActiveVertices, 2, (void*)srcRange);
	clSetKernelArgSVMPointerAltera(readActiveVertices, 3, (void*)fpgaIterNum);

	clSetKernelArgSVMPointerAltera(gatherActiveVertices, 0, (void*)activeVertices);	
	clSetKernelArgSVMPointerAltera(gatherActiveVertices, 1, (void*)activeVertexNum);
	clSetKernelArgSVMPointerAltera(gatherActiveVertices, 2, (void*)srcRange);
	clSetKernelArgSVMPointerAltera(gatherActiveVertices, 3, (void*)fpgaIterNum);
...
void launchFPGA(CSR * csr, std::vector<CSR_BLOCK*> &blkVec)
{
		...
		cl_event eventGatherActiveVertices;
		...
		for(int row = 0; row < blkNum * blkNum; row++){	
			...
			#else
			status = clEnqueueTask(queueGatherActiveVertices, gatherActiveVertices, 0, NULL, &eventGatherActiveVertices);
