	...
	)
{
	...
	clSetKernelArgSVMPointerAltera(readNgbInfo, 1, (void*)blkRpaNum);
	...
	clSetKernelArgSVMPointerAltera(readNgbInfo, 9, (void*)itNum);
...
void paddingBlk(const std::vector<CSR_BLOCK*> &blkVec){
  ...
  printf("srcStart %d, srcEnd %d \n",srcStart, srcEnd);
	...
	for(int i = 0; i <= blkVertexNum[0]; i++) blkRpaNum[i] = blkRpa[i+1] - blkRpa[i];
	//blkActiveVertexNum[0] = 1;
	#endif
}
		...
		printf("[INFO] FPGA samll partition newly created thread runtime is %f ms \n", (end - begin) * 1000);
			...
			for(int i = 0; i < blkActiveVertexNum[0]; i++){
				int v = blkActiveVertices[i];
  		          ...
  		          //	if(v == 100) printf("num is %d",blkRpa[v - srcStart + 1] - blkRpa[v - srcStart]);
			...
			printf("FPGA task CPU run time is %lf\n", fpga_runtime);
			#else
			double begin = getCurrentTimestamp();
			...
			double end = getCurrentTimestamp();
			fpga_runtime = (end - begin);
			printf("[INFO] FPGA run time for one block is %lf ms\n", fpga_runtime * 1000);
			#endif
		{	
			double begin = getCurrentTimestamp();
			for(int row = 0; row < blkNum; row++){
	...
	for(int cordx = 0; cordx < blkNum; cordx++){
		for(int cordy = 0; cordy < blkNum; cordy++){
			...
			if(csrBlkPtr->edgeNum > max_partition_degree){
				...
				printf("fpga_partition_x %d, fpga_partition_y %d, max_partition_degree %d , vertex num %d \n",fpga_partition_x, fpga_partition_y,max_partition_degree, csrBlkPtr->vertexNum);
			}
...
int main(int argc, char **argv) {
	...
	std::cout << "[INFO] Software PR takes " << elapsedTime << " ms." << std::endl;
	...
	std::cout << "Hybrid 2-thread processing." << std::endl;
	processInit(vertexNum, edgeNum, startVertexIdx);
	elapsedTime = hybridProcessing(csr, blkVec, hybridProp, blkNum, vertexNum, edgeNum, 2);
	std::cout << "[INFO] Hybird PageRank with partition takes " << elapsedTime << " ms." << std::endl;
