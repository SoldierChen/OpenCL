...
void launchFPGA(CSR * csr, std::vector<CSR_BLOCK*> &blkVec)
{
		...
		for(int row = 0; row < blkNum * blkNum; row++){
			...
			double end = getCurrentTimestamp();
      printf("time to ensure the next active vertex %f \n",(end - begin)* 1000);
			if(!i) continue;
			#if 0
			//double begin = getCurrentTimestamp();
...
bool verify(int * cur, int * ref){
  for (int i = 0; i < vertexNum; i ++){
     if(cur[i] != ref[i])
        printf("idx %d Failed current %d ref %d \n", i, cur[i], ref[i]);
  }
  return 1;
}
...
int main(int argc, char **argv) {
	...
	begin = getCurrentTimestamp();
	singleThreadSWProcessing(csr, blkVec, swProp, blkNum, vertexNum, startVertexIdx);
	end = getCurrentTimestamp();
