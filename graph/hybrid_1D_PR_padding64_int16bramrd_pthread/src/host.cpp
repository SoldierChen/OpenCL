	...
	)
{
	...
	while(itNum[0] < MAX_ITER){
...

		{
			...
			printf("fpga partition CPU main thread run time is %lf\n", fpga_runtime);
		}
					...
					)
{
		...
		cl_event eventProcessEdge;
			...
			printf("fpga partition CPU run time is %lf\n", fpga_runtime);
			#else
			double begin = getCurrentTimestamp();
...
{	
	...
	itNum[0] = 0;
	paddingBlk(blkVec);
	if(platSeclct == 2) init_fpga(blkVec, vertexNum, edgeNum, blkNum);	
	const ScoreT base_score = (1.0f - kDamp) / vertexNum;
	while(itNum[0] < MAX_ITER){	
	#ifdef DEBUG
		...
		for(int i = 0; i < THREAD_NUM; i++){
			//int rc = pthread_create(&cpuThread[i], NULL,cpuThreadFunc, (void *)csr);
			cpuThread[i]  = std::thread(cpuThreadFunc, (void *)csr);
    		// cpu_set_t cpuset;
    		// CPU_ZERO(&cpuset);
    		// CPU_SET(i, &cpuset);
		...
		fpgaThreadFunc(csr, blkVec, blkNum);
		while(cpuThreadUp);
	...
	return (runtime);//*1.0/CLOCKS_PER_SEC;
}
