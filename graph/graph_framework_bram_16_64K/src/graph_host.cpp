...
void processReset(const int &nodeNum, const int &source){
	...
	//vertexProp[source] = 0;
	vertexProp[source] = 1.0f / nodeNum;
	activeVertexNum[0] = 1;
	activeVertices[0] = source;
}
...
int main(int argc, char ** argv){
  ...
  const double start_time_cpu = getCurrentTimestamp();
  processOnCPU(nodeNum, edgeNum);

  const double end_time_cpu = getCurrentTimestamp();
	printf("cpu runtime is: %f ms \n ",(double)(end_time_cpu - start_time_cpu)* 1000);
  for(int i = 0; i < nodeNum; i++){
	...
	std::cout << "Creating graph processing kernels." << std::endl;
	{
		...
		clSetKernelArgSVMPointerAltera(processEdge, 1, (void*)activeVertexNum);
		...
		clSetKernelArg(updateNextFrontier, 2, sizeof(int), (void*)&nodeNum);
  ...
  const double start_time = getCurrentTimestamp();
	...
	while(loop_conditon){
		...
		int totalActiveVertexNum = activeVertexNum[1];
		
		memcpy (activeVertices, nextActiveVertices, activeVertexNum[1] * sizeof(int));
		activeVertexNum[0] = activeVertexNum[1];
		
		#ifdef BFS
			std::cout << "BFS level = " << itNum[0] << " next frontier = " << totalActiveVertexNum << std::endl;
			itNum[0]++;
			loop_conditon = (activeVertexNum[0] > 0)? 1 : 0;
		#endif
		#ifdef SSSP
			std::cout << "BFS level = " << itNum[0] << " next frontier = " << totalActiveVertexNum << std::endl;
			itNum[0]++;
		...
		#ifdef PR
			itNum[0]++;
			loop_conditon = (semaphore[1] > epsilon)? 1 : 0;
			std::cout << "PageRank iteration = " << itNum[0] << " next frontier = " << totalActiveVertexNum << std::endl;
			printf ("%lf\n",semaphore[1]);
