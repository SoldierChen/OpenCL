...
// Read graph from edge based files
		Graph* createGraph(const std::string &gName, const std::string &mode){
			...
			else if(mode == "rmat") dir = "/home/xinyuc/hddwork/krongen/";
			else {
			...
			if(gName == "dblp"){
				...
				gptr = new Graph(dir + "rmat-17-32.txt");
						...
						else if(gName == "rmat-17-4"){
				gptr = new Graph(dir + "rmat-17-4.txt");
			}
			else if(gName == "rmat-17-8"){
				gptr = new Graph(dir + "rmat-17-8.txt");
			}
			else if(gName == "rmat-17-16"){
				gptr = new Graph(dir + "rmat-17-16.txt");
			}
			else if(gName == "rmat-17-64"){
				gptr = new Graph(dir + "rmat-17-64.txt");
			}
			else if(gName == "rmat-17-128"){
				gptr = new Graph(dir + "rmat-17-128.txt");
			}
			else if(gName == "rmat-17-256"){
				gptr = new Graph(dir + "rmat-17-256.txt");
			}
       else if(gName == "bio-mouse-gene"){
   gptr = new Graph(dir + "bio-mouse-gene.edges");
  }
  else if(gName == "bio-human-gene1"){
   gptr = new Graph(dir + "bio-human-gene1.edges");
  }
  else if(gName == "bio-human-gene2"){
   gptr = new Graph(dir + "bio-human-gene2.edges");
  }
      else if(gName == "soc-LiveMocha"){
  		  gptr = new Graph(dir + "soc-LiveMocha.mtx");
			}
			else if(gName == "soc-buzznet"){
				gptr = new Graph(dir + "soc-buzznet.mtx");
			}
			else if(gName == "soc-themarker"){
				gptr = new Graph(dir + "soc-themarker.edges");
			}
			else{
...
int main(int argc, char ** argv){
	...
	int source = 365723;
	std::string mode = "rmat";
	std::string graphName = "rmat-17-256";
  int padding_num_argv = 4;
	if(argc == 3){
    graphName = argv[1];
    padding_num_argv = atoi(argv[2]);
  }
  std::cout << graphName <<" Padding number is: " << padding_num_argv <<std::endl;
	...
	outEdges           = (int*) clSVMAllocAltera(context, 0, sizeof(int) * edgeNum * 40, 1024);
	...
	//processInit(csr, nodeNum, edgeNum, source);	
	 padding(csr, nodeNum, edgeNum, padding_num_argv);	
	int padding_num = 0;
	for(int i = 0; i < nodeNum; i++){
		int start = outRowPointerArray[i];
		int num = outdeg_padding[i];
		//printf("\n vertex %d num %d start %d \n",i,num,start);
		for(int j = start; j < start + num; j ++){
			if(outEdges[j] == -1) padding_num ++;
			//printf("%d \t", outEdges[j]);
		}
		if(i == (nodeNum - 1)) printf("%d \n", start + num);
	}
	printf("original edgeNum %d padding num %d total edges %d \n",edgeNum, padding_num, (edgeNum + padding_num));
	
	//for(int i = 0; i < 1000; i ++) printf("%d \t", cia[i]);
#if 0
  ...
  double time = (end_time - start_time);
	printf(" FPGA runtime not using event is: %f ms\n", time*1000);
  ...
  clGetEventProfilingInfo(eventProcessEdge, CL_PROFILING_COMMAND_START, sizeof(time_start), &time_start, NULL);
  clGetEventProfilingInfo(eventProcessEdge, CL_PROFILING_COMMAND_END, sizeof(time_end), &time_end, NULL);
	double total_time = time_end - time_start;
  printf(" FPGA event runtime is: %lf ms\n", total_time / 1000000);
	
  double time = (end_time - start_time);
	printf(" FPGA runtime is: %f ms\n", time*1000);
  // Copy FPGA result
	// for(int i = 0; i < nodeNum; i++){
