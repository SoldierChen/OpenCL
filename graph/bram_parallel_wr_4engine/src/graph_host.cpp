...
int main(int argc, char ** argv){
  ...
  printf("iteration %d \n", itNum[0]);	
	...
	while(loop_condition){
		...
		k0Time += (c1 - c0);
		itNum[0] ++;
    if(itNum[0] > 100) loop_condition = false;
 	}
  printf("iteration %d \n", itNum[0]);	
  ...
  for(int j = 0; j <= 100; j ++){
	  for(int i = 0; i < activeVertexNum[0]; i ++){
      if(i % 4 == 0)
	  	  dstBuffer0[activeVertices[i]] += activeVertices[i];
      if(i % 4 == 1)
	  	  dstBuffer1[activeVertices[i]] += activeVertices[i];
      if(i % 4 == 2)
	  	  dstBuffer2[activeVertices[i]] += activeVertices[i];
      if(i % 4 == 3)
	  	  dstBuffer3[activeVertices[i]] += activeVertices[i];
	  }
  }


	for(int i = 0; i < VERTEX_MAX; i++){
	//	if ((dstBuffer0[i] | dstBuffer1[i] | dstBuffer2[i] | dstBuffer3[i]) != outRowPointerArray[i])
