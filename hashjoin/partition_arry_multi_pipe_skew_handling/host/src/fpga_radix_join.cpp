...
int creatKernels(){
    ...
    kernel_skewHandle = clCreateKernel(program, "skewHandle", &status);
    if(status != CL_SUCCESS) {
        dump_error("Failed clCreateKernel.", status);
        freeResources();
        return 1;
    ...
    argvi = 0;
    clSetKernelArgSVMPointerAltera(kernel_skewHandle, argvi ++, (void*)skewTable);
    argvi = 0;
    clSetKernelArgSVMPointerAltera(kernel_hashjoin, argvi ++, (void*)skewTable);
...
int varMap(){
    ...
    status = clEnqueueSVMMap(queue_skewHandle, CL_TRUE,  CL_MAP_READ | CL_MAP_WRITE, 
        (void *)skewTable, rTableSize, 0, NULL, NULL); 
    if(status != CL_SUCCESS) {
        dump_error("Failed clEnqueueSVMMap", status);
        freeResources();
        return 1;
