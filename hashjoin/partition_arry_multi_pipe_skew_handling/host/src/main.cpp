...
cl_command_queue queue_skewHandle;

cl_kernel kernel_hashjoin;
cl_kernel kernel_relRead;
cl_kernel kernel_skewHandle;
...
int factor = 4;
unsigned int rTupleNum = 1024*256;//0x1000000/factor;//16318*1024; //16 * 1024 * 1204 ;
unsigned int sTupleNum = 1024*256;//16318*1024; //16 * 1024 * 1024;
...
int setKernelEnv(){
    ...
    queue_skewHandle = clCreateCommandQueue(context, device, CL_QUEUE_PROFILING_ENABLE, &status);
    if(status != CL_SUCCESS) {
        dump_error("Failed clCreateCommandQueue.", status);
        freeResources();
        return 1;
...
int main(int argc, char *argv[]) {
	...
	skewTable = (unsigned int*)clSVMAllocAltera(context, 0, rTableSize, 1024);
...
//    	int argvi = 0;
  ...
  //   	clSetKernelArgSVMPointerAltera(kernel_hashjoin, argvi ++, (void*)sTableReadRange);
	//		varMap();
		...
		cl_event event_hashjoin, event_relRead, event_skewHandle;
		...
		status = clEnqueueTask(queue_skewHandle, kernel_skewHandle, 0, NULL, &event_skewHandle);
		if (status != CL_SUCCESS) {
			dump_error("Failed to launch kernel.", status);
			freeResources();
			return 1;
		...
		printf("[INFO] hash table(len: 0x%x) time = %0.3f ms\n", rTupleNum, (total_time / 1000000.0) );
