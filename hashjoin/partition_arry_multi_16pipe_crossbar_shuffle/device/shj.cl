...
/*
    ...
    */
      ...
      for(int i = 0; i < 16; i ++){ 
        ...
        for(int j = 0; j < 8; j ++){  
          data_r[j] = read_channel_nb_altera(relR[j][i], &valid_r[j]);        
            ...
            //uint8 mask = (uint8)(valid_r[0] , valid_r[1] , valid_r[2] , valid_r[3] , valid_r[4] , valid_r[5] , valid_r[6] , valid_r[7] );
            //printf("%d,%d,%d,%d,%d,%d,%d,%d\n", mask.s0,mask.s1, mask.s2,mask.s3 ,mask.s4,mask.s5 ,mask.s6,mask.s7);
            //printf("idx 0X%x, num %d", (shuff_ifo.idx >> ((shuff_ifo.num-1) * 3)) & 0x7, shuff_ifo.num);

          /*
            printf("%d,%d,%d,%d,%d,%d,%d, %d\n", data_r[0].x,data_r[1].x, data_r[2].x,data_r[3].x ,data_r[4].x,data_r[5].x ,data_r[6].x,data_r[7].x);



            uint8 key_8 = (uint8)(data_r[0].x , data_r[1].x , data_r[2].x , data_r[3].x , data_r[4].x , data_r[5].x , data_r[6].x , data_r[7].x );
            uint8 r = shuffle(key_8, mask);
          ...
          for(int j = 0; j < 8; j ++){  
            if(i == 0)
            {
              printf(" %d, %d\t",data_r[j].x, valid_r[j]);
            }
          }
          */
          ...
          for(int j = 0; j < shuff_ifo.num; j ++){  
              ...
              uint key  = data_r[idx].x; 
              uint val  = data_r[idx].y;
              uint hash_idx = HASH (key,(HASHTABLE_BUCKET_NUM - 1),4);
              hashtable_l[hash_idx * HASHTABLE_BUCKET_SIZE + hashtable_bucket_cnt[hash_idx][i]][i]= data_r[idx];
    ...
    while(true){
     ...
     uchar opcode = valid_s[0] + (valid_s[1] << 1) + (valid_s[2] << 2) + (valid_s[3] << 3) + (valid_s[4] << 4) + (valid_s[5] << 5) + (valid_s[6] << 6) + (valid_s[7] << 7);
