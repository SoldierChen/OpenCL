...
int main(int argc, char *argv[]) {
  ...
  unsigned int WASTableSize = rHashTableBucketNum*hashBucketSize * 64 ;
  ...
  printf("Creating hash_build kernel\n");
    ...
    cl_uint fpga_size   = rTupleNum;
  	...
  	clSetKernelArg(kernel[0],3,sizeof(cl_uint),(void*)&fpga_size);
		...
		clSetKernelArgSVMPointerAltera(kernel[0], 5, (void*)rHashCount);
 ...
 //status = clEnqueueSVMUnmap(queue, (void *)tHist, 0, NULL, NULL); 
    ...
    */
    ...
    clSetKernelArgSVMPointerAltera(kernel[1], 0, (void*)HashTable);
		...
		clSetKernelArgSVMPointerAltera(kernel[1], 3, (void*)rHashCount);

  	clSetKernelArg(kernel[1],4,sizeof(cl_uint),(void*)&rHashTableBucketNum);
  	clSetKernelArg(kernel[1],5,sizeof(cl_uint),(void*)&hashBucketSize);
    printf("Launching the build Hash Table kernel...\n");
		status = clEnqueueSVMMap(queue, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 
