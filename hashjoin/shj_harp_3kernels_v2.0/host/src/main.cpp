...
static cl_kernel kernel1, kernel2;
static cl_kernel b1;
...
int main(int argc, char *argv[]) {
  ...
  printf("Creating hash_build kernel\n");
	{
		kernel = clCreateKernel(program, "buildHist_p1", &status);
		if(status != CL_SUCCESS) {
    ...
    printf("build Hist_p1(len: 0x%x) time = %0.3f ms\n", rTupleNum, (total_time / 1000000.0) );
    
    kernel2 = clCreateKernel(program, "buildHist_p2", &status);
		if(status != CL_SUCCESS) {
			dump_error("Failed clCreateKernel.", status);
			freeResources();
			return 1;
		} 
		clSetKernelArgSVMPointerAltera(kernel2, 0, (void*)rTable);
		clSetKernelArgSVMPointerAltera(kernel2, 1, (void*)HashTable);
		clSetKernelArg(kernel2,2, sizeof(cl_uint), (void*)&fpga_offset);
  	clSetKernelArg(kernel2,3,sizeof(cl_uint),(void*)&fpga_size);
		clSetKernelArg(kernel2,4,sizeof(cl_uint),(void*)&rHashTableBucketNum);
		clSetKernelArg(kernel2,5,sizeof(cl_uint),(void*)&hashBucketSize);
		clSetKernelArgSVMPointerAltera(kernel2, 6, (void*)tHist);
		clSetKernelArgSVMPointerAltera(kernel2, 7, (void*)rHashCount);
		
  	status =	clEnqueueNDRangeKernel(queue,kernel2,1,NULL,&gworkSize_build,&workSize,0,NULL,&event3);
    ...
    total_time = time_end - time_start;
    printf("build Hist_p2(len: 0x%x) time = %0.3f ms\n", rTupleNum, (total_time / 1000000.0) );
 
	  kernel1 = clCreateKernel(program, "buildHashTable", &status);
		if(status != CL_SUCCESS) {
			dump_error("Failed clCreateKernel.", status);
			freeResources();
			return 1;
		} 
		clSetKernelArgSVMPointerAltera(kernel1, 0, (void*)rTable);
		clSetKernelArgSVMPointerAltera(kernel1, 1, (void*)HashTable);
		clSetKernelArg(kernel1,2, sizeof(cl_uint), (void*)&fpga_offset);
  	clSetKernelArg(kernel1,3,sizeof(cl_uint),(void*)&fpga_size);
		clSetKernelArg(kernel1,4,sizeof(cl_uint),(void*)&rHashTableBucketNum);
		clSetKernelArg(kernel1,5,sizeof(cl_uint),(void*)&hashBucketSize);
		clSetKernelArgSVMPointerAltera(kernel1, 6, (void*)tHist);
		clSetKernelArgSVMPointerAltera(kernel1, 7, (void*)rHashCount);
		printf("Launching the build Hash Table kernel...\n");
		status =	clEnqueueNDRangeKernel(queue,kernel1,1,NULL,&gworkSize_build,&workSize,0,NULL,&event2);
		if (status != CL_SUCCESS) {
			dump_error("Failed to launch kernel.", status);
			freeResources();
			return 1;
    ...
    total_time = time_end - time_start;
    printf("build table(len: 0x%x) time = %0.3f ms\n", rTupleNum, (total_time / 1000000.0) );
