                      ...
                      )
{
  ...
  uint2 buffer[32][16]; //ulong4 ulong_buffer; //ulong buffer[BUFF_DEPTH];
  char tuple_num[16] = {0}; //int buff_avaialbe_num = 0;
  int i = 0; 
  char rd_counter[16]={0}, wr_counter[16]={0};
      ...
      #pragma unroll 16
        ...
        // each collect engine do their work
          ...
          if (tuple_num[i] <= (32- 16)){
              #pragma unroll 8
                ...
                engine_finish[i] = valid_r[0] | valid_r[1] | valid_r[2] | valid_r[3]| 
                                   valid_r[4] | valid_r[5] | valid_r[6] | valid_r[7] | tuple_num[i];
                #pragma unroll 8
                for(int j = 0; j < 8; j ++){  
                  if(valid_r[j]) {
                    buffer[wr_counter[i]][i] = data_r[j]; 
                    wr_counter[i] = ( wr_counter[i] + 1)&(32-1);
                    tuple_num[i]++;
                  }
                }
            }
          ...
          if(tuple_num[i] >= 1)
            {
              tuple_num[i]--;
              uint2 tuple_out = buffer[rd_counter[i]][i]; //ulong_buffer.s0;
              rd_counter[i] = ( rd_counter[i]+1)&(32-1);//ulong_buffer >>= 64;
              //debug_out[i++] = tuple_out;
              write_channel_altera(buildCh[i], tuple_out);
            }
