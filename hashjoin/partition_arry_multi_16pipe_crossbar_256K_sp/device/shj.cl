...
#define ENDFLAGBOOL true

channel uint2 relR[8][16] __attribute__((depth(8)));
channel uint2 relS[8][16] __attribute__((depth(8)));
channel bool relRendFlagCh[8][16] __attribute__((depth(8)));
//channel bool relSendFlagCh[8][16] __attribute__((depth(8)));
 ...
 //   for(int i = rTableOffset; i < rTableOffset + rTableReadNum; i ++){
    for(int i = 0; i < rTableReadNum; i += 8){
      ...
      #pragma unroll 8
        ...
        #pragma unroll 16
          for(int i = 0; i < 16; i ++){
            write_channel_altera(relRendFlagCh[k][i], ENDFLAGBOOL);
          }
        }
...
/*
    #pragma unroll 8
      ...
      #pragma unroll 16
        for(int i = 0; i < 16; i ++){
          write_channel_altera(relSendFlagCh[k][i], ENDFLAGBOOL);
        }
      }
*/
...
*/
    ...
    bool valid_r[8][16];
    bool endFlagData[8][16];
    bool valid_endflag[8][16];
      ...
      for(int i = 0; i < 16; i ++){  
        ...
        while(true){
          ...
          */
            #pragma unroll 8
              for(int j = 0; j < 8; j ++){  
                bool tmp = read_channel_nb_altera(relRendFlagCh[j][i], &valid_endflag[j][i]);
                if(valid_endflag[j][i]) endFlagData[j][i] = tmp;
              }
              ...
              bool endFlag = endFlagData[0][i] & endFlagData[1][i] & endFlagData[2][i] & endFlagData[3][i] & 
                               endFlagData[4][i] & endFlagData[5][i] & endFlagData[6][i] & endFlagData[7][i] ;
              
              if(endFlag && !engine_finish[i]) break; 
