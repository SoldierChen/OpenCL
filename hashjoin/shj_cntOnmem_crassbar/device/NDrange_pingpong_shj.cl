...
__attribute__((reqd_work_group_size(128,1,1)))
__attribute__((num_compute_units(1)))
__kernel void update_table( __global uint * restrict rHashTable, __global bucket_cell_t * restrict WASTable, __global uint * restrict rHashCount, const uint hashBucketSize,__global uint * restrict l_cnt)
{
  uint numWorkItems = get_global_size(0);
  uint tid          = get_global_id(0);
  uint iteration = tid;
