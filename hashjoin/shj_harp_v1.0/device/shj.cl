  ...
  while (tid < size)
	{
		...
		key  = rtable_uint2.x; val  = rtable_uint2.y;
		      ...
		      hash_index = sim_hash(key, rHashTableBucketNum);//djb2_hash(key) & (rHashTableBucketNum-1); // real bucket to populate
	        //tHist[hash_index] ++ ;
...
// write result to hash table 
  ...
  while (iteration < size){
        ...
        key  = rtable_uint2.x; val  = rtable_uint2.y;
        hash_index = sim_hash(key, rHashTableBucketNum); //djb2_hash(key) & (rHashTableBucketNum-1);
        if(tid != 0){
