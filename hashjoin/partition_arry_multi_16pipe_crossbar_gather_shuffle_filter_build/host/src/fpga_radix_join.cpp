...
int creatKernels(){
    ...
    kernel_filter = clCreateKernel(program, "filter", &status);
    if(status != CL_SUCCESS) {
        dump_error("Failed clCreateKernel.", status);
        freeResources();
        return 1;
    ...
    // argvi = 0;
    // clSetKernelArgSVMPointerAltera(kernel_gather, argvi ++, (void*)matchedTable);
    // clSetKernelArgSVMPointerAltera(kernel_gather, argvi ++, (void*)rTableReadRange);
    // clSetKernelArgSVMPointerAltera(kernel_gather, argvi ++, (void*)sTableReadRange);
    // argvi = 0;
    // clSetKernelArgSVMPointerAltera(kernel_filter, argvi ++, (void*)matchedTable);
    // clSetKernelArgSVMPointerAltera(kernel_filter, argvi ++, (void*)rTableReadRange);
    // clSetKernelArgSVMPointerAltera(kernel_filter, argvi ++, (void*)sTableReadRange);
    argvi = 0;
